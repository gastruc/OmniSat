...
from models.networks.encoder.utils.convlstm import ConvLSTM, BConvLSTM
from models.networks.encoder.utils.ltae import LTAE2d


modality_to_channels = {'aerial': 4, 's2': 10, 's1-asc': 3, 's1-des': 2}
...
class UTAE(nn.Module):
    ...
    def forward(self, x, return_att=False):
        ...
        batch_positions = x['_'.join([self.modality, "dates"])]
        pad_mask = (
...
class UpConvBlock(nn.Module):
    ...
    def forward(self, input, skip):
        out = self.up(self.conv_up(input, output_size = skip.shape))
