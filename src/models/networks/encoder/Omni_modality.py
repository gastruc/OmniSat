...
class OmniModule(nn.Module):
    """
    Omni module with a different masked token by modality
    """
    def __init__(self, 
                 projectors: dict = {},
                 modalities: list = [],
        ...
        self.modality_encoding = {key: torch.eye(len(projectors.keys()))[i] for i, key in enumerate(projectors.keys())}
        embed_dim = embed_dim + len(projectors.keys())
    ...
    def forward_proj(self, x):
        ...
        for modality in self.modalities:
            ...
            tokens.append(torch.cat((out['_'.join(['tokens', modality])]  + self.pos_embed[:, 1:, :]), 
                                    self.modality_encoding.repeat(out['_'.join(['tokens', modality])].shape[0],
                                                                   out['_'.join(['tokens', modality])].shape[1],1)), dim=-1)
