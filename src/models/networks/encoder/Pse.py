...
def masked_mean(x, mask):
    ...
    mask_sum = torch.where(mask_sum > 0, mask_sum, torch.tensor(1.0))
    out = torch.where(mask_sum > 0, out.sum(dim=-1) / mask_sum, torch.tensor(0.0)) # to handle division by 0
...
modality_to_channels = {'aerial': 4, 's2': 10, 's1-asc': 2, 's1-des': 2, 's1': 2, 's1-mono': 3, 's2-mono': 12, 's2-mae': 10}
...
class Pse(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        mask = torch.zeros((input_shape[0], input_shape[2] * input_shape[3]))
        for i in range (len(input)):
            if torch.max(input[i]) > 0:
                mask[i] += torch.ones((input_shape[2] * input_shape[3]))
