...
from models.networks.encoder.utils.utils_ViT import Block, CrossBlock


class OmniModule(nn.Module):
    """
    Omni module with absolute positional encoding
    """
    def __init__(self, 
                 projectors: dict = {},
                 modalities: list = [],
        ...
        self.blocks = nn.ModuleList([
            ...
            for i in range(depth)] + [CrossBlock(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, modalities=modalities,
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, num_patches=self.num_patches)])
